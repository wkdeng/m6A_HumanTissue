"""
kept original read information
"""
import os
import sys
import pysam
...
def filter_bam_multihits(filename, max_tags, max_hits, out_dir, read_tagger_method, strandness):
	...
	for i in args:
		msg += "%s = %s \n" % (i, values[i])
	logger.info(msg)
	def read_tagger(x): return read_tagger_collection(
		x, method=read_tagger_method)
	...
	if not \
		...
		for read in in_bam:
			...
			tagged_read.query_qualities = read.query_qualities
		...
		# sorting
		pysam.sort('-m', '4G', '-@', '3', '-T',
		           os.path.dirname(sorted_ubam_fn), '-o', sorted_ubam_fn, ubam_fn)
		os.remove(ubam_fn)
		pysam.sort('-m', '4G', '-@', '3', '-T',
		           os.path.dirname(sorted_mbam_fn), '-o', sorted_mbam_fn, mbam_fn)
		os.remove(mbam_fn)
		...
		logger.info(
                    'Unique reads = %s;  ' % unique_counter +
                				'Multi reads = %s (%.2f %%)' %
                				(multi_counter, float(multi_counter) /
                				 (multi_counter+unique_counter)*100)
                )
	else:
	...
	if max_tags > 0:
		...
		filter_bam_maxtags(os.path.join(out_dir, 'unique.sorted.collapsed.bam'),
...
def collapse_stack(stack, collapse_dict, max_tags):
	...
	for seq in new_alignment_dict:
		...
		is_collapsed = [
			True if x in collapse_dict else False for x in this_alignment_qname_list]
		...
		if any(is_collapsed):
			...
			target_alignment = [new_alignment_dict[seq][this_alignment_qname_list.index(
				x)] for x in target_alignment_qname]
		else:
...
def filter_bam_maxtags(obam_fn, ibam_fn, max_tags=1):
	...
	for chr in chr_list:
		...
		for read in ibam.fetch(chr):
			...
			if read.positions[0] > last_pos:
				new_alignment_list, collapse_dict = collapse_stack(
					stack, collapse_dict, max_tags)
	...
	#pysam.sort(obam_fn)
	pysam.index(obam_fn)
	logger.info('Input = %s; Output = %s; Redundancy = %.2f' % (
		input_counter, output_counter, 1-float(output_counter)/input_counter))
	return
...
if __name__ == '__main__':
	# try:	
		...
		if not os.path.isdir(out_dir):
			os.mkdir(out_dir)
		tag_method = sys.argv[3]
		max_hits = int(sys.argv[4])
		## Note: if specified max_tags, need pre-sorted bam
		max_tags = int(sys.argv[5])

		#logger = logging.getLogger('CLAM.Preprocessor')
		logger.info('start')
		logger.info('run info: %s' % (' '.join(sys.argv)))

		filter_bam_multihits(in_bam, max_tags, max_hits, out_dir, tag_method, 'same')
