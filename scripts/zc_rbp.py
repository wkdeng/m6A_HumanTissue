...
## @create date 2022-03-15 17:30:55
## @modify date 2022-03-15 17:30:55
## @desc [description]
###
import pandas as pd
...
stop_codon.loc[stop_codon['Sign'] < 0, 'START'] = [max(0, x-8000) for x in
                                                   stop_codon.loc[stop_codon['Sign'] < 0, 'START']]
stop_codon.drop_duplicates(
    subset=['CHR', 'START', 'END', 'STRAND'], inplace=True)
...
for file_ in os.listdir('WGCNA/RBP_peaks/ENCODE_peaks'):
    if file_.endswith('-human.sorted.bed'):
        all_rbp.append(file_.replace('-human.sorted.bed', ''))
...
def get_peak_around_sc(peak_file, peak2gene=None):
    ...
    if peak2gene is None:
        ...
        peak2gene = pd.read_csv(peak2gene, sep='\t',
                                header=None, index_col=0, names=['gene'])
    ...
    peak_around_sc.iloc[:, 5] = [1 if x == '+' else -
                                 1 for x in peak_around_sc.iloc[:, 5]]
    peak_around_sc.iloc[:, 11] = [1 if x == '+' else -
                                  1 for x in peak_around_sc.iloc[:, 11]]
    peak_around_sc.iloc[:, 10] = (
        peak_around_sc.iloc[:, 7]+peak_around_sc.iloc[:, 8])/2
    peak_around_sc.iloc[:, 10] = peak_around_sc.iloc[:, 10].astype(int)
    peak_around_sc['M'] = (peak_around_sc.iloc[:, 10] -
                           peak_around_sc.iloc[:, 4])*peak_around_sc.iloc[:, 11]
...
def count_ol(RBP, target_rbp, target_rbp_name):
    _, rbp_dict = get_peak_around_sc(
        'WGCNA/RBP_peaks/ENCODE_peaks/%s-human.sorted.bed' % RBP)
    rbp_dict = {x: rbp_dict[x]
                for x in [y for y in rbp_dict.keys() if y in target_rbp]}
    target_rbp2 = {x: target_rbp[x]
                   for x in [y for y in rbp_dict.keys() if y in target_rbp]}
    max_ele = 0
    for key in rbp_dict:
        max_ele = max(len(target_rbp2[key]), max(len(rbp_dict[key]), max_ele))
    if len(target_rbp2) == 0:
        return
    table = 'TX\t'
    for list_ in ['Target', 'RBP']:
        for i in range(1, max_ele+1):
            table += '%s%s\t' % (list_, i)
    table += 'count1\tcount2\n'
    for tx in target_rbp2:
        table += tx+'\t'
        for list_ in [target_rbp2, rbp_dict]:
            for i in range(max_ele):
                if len(list_[tx]) <= i:
                    table += 'NA\t'
                else:
                    table += '%s\t' % list_[tx][i]
        table += '%s\t%s\n' % (len(target_rbp2[tx]), len(rbp_dict[tx]))
    if not os.path.isdir('WGCNA/zc/encode_pairwise_count'):
        os.mkdir('WGCNA/zc/encode_pairwise_count')
    with open('WGCNA/zc/encode_pairwise_count/%s_%s.txt' % (RBP, target_rbp_name), 'w') as fo:
        fo.write(table.strip())
        fo.close()


def count_module_ol(target_rbp_name):
    _, target_rbp = get_peak_around_sc(
        'WGCNA/RBP_peaks/ENCODE_peaks/'+target_rbp_name+'-human.sorted.bed')
    for rbp in all_rbp:
        if not os.path.exists('WGCNA/zc/encode_pairwise_count/%s_%s.txt' % (rbp, target_rbp_name)):
            count_ol(rbp, target_rbp, target_rbp_name)

pool = Pool(processes=20)
ret = pool.map(count_module_ol, all_rbp)
pool.close()
pool.join()
