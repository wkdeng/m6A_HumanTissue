...
#  * @create date 2019-09-20 10:24:37
#  * @modify date 2019-09-20 10:24:37
#  * @desc [description]
#  */
# /usr/bin/env python2
import sys
...
for line in open(os.path.join(cur_dir,'%s_ensGene.txt'%genome)):
    ...
    if (longtx[genename][0]!=tx or cdsstart==cdsend):
        continue
    if (tx not in coding or coding[tx]!='protein_coding'):
        continue
    if (chr not in peak):
        continue
    for i in range(len(exonstart)):
...
total_peak_num = 0
for gene in tx_with_peak:
    total_peak_num = total_peak_num + tx_with_peak[gene]

output = sys.stdout
for i in range(len(peakbin)):
    output.write(str(peakbin5[i]*1.0/txnum)+'\t'+str(peakbin5[i]*1.0/total_peak_num)+'\t'+str(peakbin5[i])+'\n')
for i in range(len(peakbin)):
    output.write(str(peakbincds[i]*1.0/txnum)+'\t'+str(peakbincds[i]*1.0/total_peak_num)+'\t'+str(peakbincds[i])+'\n')
for i in range(len(peakbin)):
    output.write(str(peakbin3[i]*1.0/txnum)+'\t'+str(peakbin3[i]*1.0/total_peak_num)+'\t'+str(peakbin3[i])+'\n')
